...
def test_reservation_import_confirm_zooking(mocker: MockerFixture, fake_user):
    ...
    message_body = to_json(MessageFactory.create_confirm_reservation_message(
        ex_reservation=test_reservation,
    ))

    mock_requests_put = Mock()
    mocker.patch("requests.put", mock_requests_put)
    ...
    update_reservation_spy.assert_called_once_with(
        Service.ZOOKING, reservation_in_db.internal_db, ReservationStatus.CONFIRMED
    )
